...
func _enter_tree() -> void:
	...
	move_forward.text = options.get_value("input", "move_forward").to_upper()
	move_left.text = options.get_value("input", "move_left").to_upper()
	move_backward.text = options.get_value("input", "move_backward").to_upper()
	move_right.text = options.get_value("input", "move_right").to_upper()
...
func _flush_active_button() -> void:
	active_input_button.button_pressed = false
	active_input_button = null
	active_input_button_old_text = ""

func _swap_await_message_with_input(text: String = active_input_button_old_text) -> void:
	...
	if toggled_on:
		...
		for button in input_button_group:
			if button.button_pressed:
				...
				button.text = "Awaiting keyboard input..."

func _unhandled_key_input(event: InputEvent) -> void:
	if event.pressed:
		if event.keycode == KEY_ESCAPE:
			_swap_await_message_with_input()
			_flush_active_button()
		else:
			var event_lower_case: String = event.as_text().to_lower()

			if event is InputEventKey:
				OptionsManager.options_to_buffer("input", active_input_button.get_meta("action"), event_lower_case)

			_option_changed_checker(indicators[active_input_button.get_meta("index")], "input", active_input_button.get_meta("action"), event_lower_case)

			_swap_await_message_with_input(event.as_text())
			_flush_active_button()
